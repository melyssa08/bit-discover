window.addEventListener("load", function () {
  $("#jobsVisualization-header").load(
    "/page_candidates/side_and_navbar/index.html",
...
function teste() {
  ...
  resultado.map((resul) => {
    ...
    $("#jobsVisualization-cards").append(
      ...
      resul.id +
      `">SAIBA MAIS</button>
             <svg xmlns="http://www.w3.org/2000/svg" width="35" height="30" id="jobsVisualization-icon-like` + resul.id + `" fill="currentColor" class="bi bi-heart-fill" viewBox="0 0 16 16">
      ...
      `</li>
                      ...
                      <ul class="jobsVisualization-activities-required">
                        <li>` +
      resul.softskills.concat(resul.hardskills).join(', ')
        `</li>
                      </ul>
  ...
  // }).then(() => {
  for (i = 4; i < 6; i++) {
    var iconLike = document.getElementById(`jobsVisualization-icon-like${i}`);
